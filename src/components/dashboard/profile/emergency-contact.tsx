...
const EmergencyContact = ({
  ...
  const updateOrCreateUserInfo = async (e: React.FormEvent) => {
    ...
    if (!last_name || last_name.length < 2)
      return toast.error("Last name is invalid");
    ...
    if (!birth_date || birth_date.length < 2)
      return toast.error("Birthdate is invalid");
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 lg:gap-x-16 gap-8 w-full">
        ...
        {contactInfo.status !== ProfileStatus.VERIFIED && (
          <div className="flex flex-col gap-3 w-full ">
              ...
              {(contactInfo.status === ProfileStatus["INCOMPLETE"] ||
                ...
                formData.front_id && (
                  <Button
                    ...
                    variant={"secondary"}
                    onClick={() =>
                      setFormData((prev) => ({ ...prev, front_id: "" }))
                    }
                  >
                    Reupload
                  </Button>
            ...
            <UploadButton
              ...
              content={{
                button(props) {
                  if (props.ready)
                    return (
                      <div className="w-40 h-ful flex items-center gap-2 h-full bg-secondary justify-center">
                        <UploadCloud size={18} />
                        Upload
                      </div>
                    );
            ...
            <small className="text-muted-foreground">
              Please provide the front of your ID for identity verification.
            </small>
          </div>
        )}
        {contactInfo.status !== ProfileStatus["VERIFIED"] && (
          <div className="flex flex-col gap-3 w-full ">
            ...
            <small className="text-muted-foreground">
              Please provide the back of your ID for identity verification.
            </small>
          </div>
        )}
      </div>
