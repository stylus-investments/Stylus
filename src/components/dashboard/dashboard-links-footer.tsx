        ...
        <div className='lg:hidden fixed z-50 w-screen right-0 padding border-t h-16 bottom-0 bg-card'>
            <div className='flex items-center w-full text-muted-foreground h-full'>
                {dashboardLinks.map((link, i) => (
                    <Link href={link.link} key={i} className={`flex flex-col items-center cursor-pointer ${currentPage === link.label.toLocaleLowerCase() && "text-foreground bg-muted"} p-1 justify-center rounded-md w-full`}>
