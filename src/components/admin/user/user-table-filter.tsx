...
const UserTableFilter = ({
  ...
  const handleStatusChange = (status: string, emergency_contact: boolean) => {
    const url = new URL(window.location.href);
    if (status) {
      if (emergency_contact) {
        url.searchParams.set("emergency_contact_status", status); // Set the new page value
      } else {
        url.searchParams.set("status", status); // Set the new page value
      }
    } else {
      if (emergency_contact) {
        url.searchParams.delete("emergency_contact_status");
      } else {
  ...
  return (
    <div className="flex items-center gap-5 justify-end">
      <div className="flex flex-col gap-2">
        ...
        <Select
          value={filter.emergency_contact_status || "all"}
          onValueChange={(val) => {
            if (val === "all") {
              handleStatusChange("", true);
            } else {
              handleStatusChange(val, true);
            }
          }}
        >
