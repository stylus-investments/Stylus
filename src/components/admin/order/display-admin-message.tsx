...
const DisplayAdminMessages = ({
  ...
  const { data: order } = trpc.message.getOrderMessages.useQuery(
      ...
      <AlertDialogContent className="w-full">
        <div className="gap-3 flex flex-col w-full sm:w-auto">
          ...
          <div className="flex justify-center items-center gap-2 sm:gap-5">
            <ToggleOrderConversation
              orderType={orderType}
              orderID={orderID}
              closed={order?.closed}
            />
            <InvalidOrder orderID={orderID} orderType={orderType} />
            <CompleteOrder orderID={orderID} orderType={orderType} />
            <div
              className="flex flex-col gap-1 items-center p-2 hover:bg-muted text-muted-foreground hover:text-foreground rounded-full justify-center min-w-16 h-16 cursor-pointer"
